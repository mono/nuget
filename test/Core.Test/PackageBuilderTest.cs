	  ...
	  <dependency id=""X"" />
		...
		public void VersionFormatIsPreserved()
		{
			...
			var dependencySet = new PackageDependencySet(null, new[] {
				new PackageDependency("B", new NuGetVersionRange(new NuGetVersion("1.0"), true)),
				new PackageDependency("C", new NuGetVersionRange(new NuGetVersion("1.0"), false, new NuGetVersion("5.0"), false))
			});
	...
	<dependencies>
	  ...
	  <dependency id=""C"" version=""(1.0, 5.0)"" />
		...
		public void SavingPackageWithDuplicateDependenciesThrows()
		{
			...
			var dependencySet = new PackageDependencySet(null, new[] {
				new PackageDependency("B", new NuGetVersionRange(lowerBound: new NuGetVersion("1.0"), includeLowerBound: true)),
				new PackageDependency("B", new NuGetVersionRange(lowerBound: new NuGetVersion("1.0"), upperBound: new NuGetVersion("5.0"), includeLowerBound: false))
			});
		...
		public void SavingPackageWithInvalidDependencyRangeThrows()
		{
			...
			var dependencySet = new PackageDependencySet(null, new[] {
				new PackageDependency("B", new NuGetVersionRange
				(
					lowerBound: new NuGetVersion("1.0"),
					upperBound: new NuGetVersion("1.0")
				))
			});
  ...
  <metadata>
	...
	<dependencies>
		...
		<dependency id=""B"" version=""[1.0, 2.5)"" />
			...
			IDictionary<string, NuGetVersionRange> dependencies = dependencySet.Dependencies.ToDictionary(p => p.Id, p => p.VersionRange);
			// <dependency id="A" version="[1.0]" />
			Assert.True(dependencies["A"].Lower.IncludeBound);
			Assert.True(dependencies["A"].IsMaxInclusive);
			Assert.Equal(new NuGetVersion("1.0"), dependencies["A"].Lower.Bound);
			Assert.Equal(new NuGetVersion("1.0"), dependencies["A"].Upper.Bound);
			// <dependency id="B" version="[1.0, 2.5)" />
			Assert.True(dependencies["B"].Lower.IncludeBound);
			Assert.False(dependencies["B"].IsMaxInclusive);
			Assert.Equal(new NuGetVersion("1.0"), dependencies["B"].Lower.Bound);
			Assert.Equal(new NuGetVersion("2.5"), dependencies["B"].Upper.Bound);
		}
  ...
  <metadata>
	...
	<dependencies>
		<group targetFramework=""sl4"">
			...
			Assert.Null(dependencies[0].VersionRange);
		}
  ...
  <metadata>
	...
	<dependencies>
		...
		<group targetFramework=""net40-client"">
			...
			Assert.Null(dependencies1[0].VersionRange);
			...
			Assert.Null(dependencies2[0].VersionRange);
...
<package>
  <metadata>
	...
	<requireLicenseAcceptance>false</requireLicenseAcceptance>
		...
		public void ValidateDependenciesThrowsIfAnyDependencyForAStableReleaseIsPrerelease(NuGetVersionRange versionRange)
		{
			// Arrange
		...
		[Theory]
		[PropertyData("InvalidDependencyData")]
		public void ValidateDependenciesDoesNotThrowIfDependencyForAPrereleaseVersionIsPrerelease(NuGetVersionRange versionRange)
		{
			// Arrange
			var dependencies = new[] {
		...
		public void ValidateDependenciesDoesNotThrowIfDependencyForAStableVersionIsStable()
		{
			// Arrange
			var dependencies = new[] {
				new PackageDependency("A", new NuGetVersionRange(new NuGetVersion("1.0.0"))),
				new PackageDependency("B", new NuGetVersionRange(lowerBound: new NuGetVersion("1.0.1"), upperBound: new NuGetVersion("1.2.3"))),
			};
			var packageVersion = new NuGetVersion("1.0.0");
		...
		public static IEnumerable<object[]> InvalidDependencyData
		{
			get
			{
				...
				yield return new object[] { new NuGetVersionRange(prereleaseVer) };
				yield return new object[] { new NuGetVersionRange (lowerBound: prereleaseVer, upperBound: version) };
				yield return new object[] { new NuGetVersionRange (lowerBound: version, upperBound: prereleaseVer, includeUpperBound: true) };
				yield return new object[] { new NuGetVersionRange (lowerBound: prereleaseVer, upperBound: prereleaseVer, includeLowerBound: true) };
			}
		}
